...
原理是基于LinkedHashMap 的排序特性来设计的：

简说：

1：当存入一个元素是 会放在链表的末尾  1 2 3 4 5 6 这样排序来存储

2：当get其中一个元素时，该元素就会被放在链表的末尾，

3：当再次存入一个元素时，会判断下当前大小，是否已经到了设置的最大上限值，如果到了就会从链表

​	  的第一个元素开始删除缓存，（**这个就是LruCache 最多使用不会被删除，也是使用之后为什么元素被放在   		链表末尾的原因**）

## DiskLruCache 使用方法

```java
//第一种 通过 DiskLruCache 提供的输入流 进行添加缓存
...
//创建DiskLruCache
cache.mDiskLruCache = DiskLruCache.open(new File(cachePath), 1, 1, maxCacheSize);

//获取存储的文件名称
 public String getEditFileName(String url) {
        DiskLruCache.Editor editor = null;
        try {
            if (mDiskLruCache == null) {
                return null;
            }
            editor = mDiskLruCache.edit(getKey(url));
            if (editor != null && mEditorMap.putIfAbsent(url, editor) == null) {
                return editor.getOutputFileName(0);
            }
        } catch (IOException e) {
            MLog.e(TAG, "Failed to get edit filename", e);
        }
        return null;
    }
}
...
//通过获取的文件名称 来下载内容
mDownloader.download(url, destFilename, isWifiRequired);

//下载成功之后提交 并刷新 DiskLruCache
 @Override
    public boolean completeEdit(String url, boolean success) {
        DiskLruCache.Editor editor = mEditorMap.get(url);
        mEditorMap.remove(url);
        if (editor != null) {
            try {
                if (success) {
                    editor.commit();
                ...
                MLog.e(TAG, "Fail to commit file cache", e);
                return false;
...
好，到这里LrcCache和DiskLruCache的实现原理都讲完了，这两个类在主流的图片框架Fresco、Glide和网络框架Okhttp等都有着广泛的应用，后续的文章后继续分析LrcCache和DiskLruCache 在这些框架里的应用。
...
> getRowBytes()返回的是每行的像素值，乘以高度就是总的像素数，也就是占用内存的大小。 getAllocationByteCount()与getByteCount()的返回值一般情况下都是相等的。只是在图片 复用的时候，getAllocationByteCount()返回的是复用图像所占内存的大小，getByteCount()返回的是新解码图片占用内存的大小。
...
![img](../../../art/三级缓存/1613c53de65edfd2)

可以发现reuseBitmap的getAllocationByteCount()和getByteCount()返回不一样，getAllocationByteCount()返回的是复用bitmap占用内存的大小， getByteCount()返回的是新的reuseOptions实际解码占用的内存大小。

注意在复用图片的时候，options.inMutable必须设置为true，否则无法进行复用，如下所示：

![img](../../../art/三级缓存/1613c53deee1ec25)
...
public class ImageLoader {
	...
	**/
    public ImageLoader() {
        ...
        //创建lurcache  返回每一个bitmap 的大小，用于lurcache 真实计算文件大小
...
LruCcache 内部使用了LinkedHashMap （最少使用原则，最少使用被放在链表前面）



## 图解

![1593411111330](C:\Users\sundu\AppData\Roaming\Typora\typora-user-images\1593411111330.png)
