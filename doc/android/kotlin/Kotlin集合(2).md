...
![img](https:////upload-images.jianshu.io/upload_images/1233356-765b2cc4ab0ee015.png?imageMogr2/auto-orient/strip|imageView2/2/w/1178/format/webp)

螢幕快照 2017-06-29 14.57.45.png

在接口`interface Map`中，K是键值的类型，V是对应的映射值的类型。这里的`out V`表示类型为V或V的子类。这是泛型的相关知识，我们将在下一章节中介绍。

其中，`Entry`中保存的是Map的键值对。
...
访问`val values: Collection`获取Map中的所有值的Collection。这个值的集合可能包含重复值。
...
`Map.Entry`的操作符函数，分别用来直接访问key和value。
...
#### `Iterable>.toMap(destination: M): M`
